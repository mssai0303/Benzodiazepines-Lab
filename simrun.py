# =====================================================================
#  simrun.py  --  HELPER FILE (you don't edit this)
# ---------------------------------------------------------------------
#  Small helpers that main.py uses to (1) set up recordings, (2) run the
#  simulation, and (3) draw/save the result plots. You can ignore the
#  details; just know main.py leans on this file to do those jobs.
# =====================================================================
...
def show_output(soma_v_vec, dend_v_vec, tuft_v_vec, t_vec, spike_times, plotflag, fstem, new_fig=True):
    ...
    # list(...) keeps this working across matplotlib versions
    soma_plot = pyplot.plot(list(t_vec), list(soma_v_vec), color='black')
    dend_plot = pyplot.plot(list(t_vec), list(dend_v_vec), color='blue')
    tuft_plot = pyplot.plot(list(t_vec), list(tuft_v_vec), color='red')
    ...
    if (spike_times.size()>0):
        pyplot.vlines(list(spike_times), i + 0.5, i + 1.5)
    pyplot.xlabel('time (ms)')
    pyplot.ylabel('Cell #')
    pyplot.ylim([0,5])
